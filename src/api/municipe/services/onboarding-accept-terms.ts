...
// Entrada esperada: { version: string, contentHash: string } no body.
// - Verifica token e role do usuário.
// - Valida o body com Zod.
// - Busca o termo ativo e compara versão + hash.
...
export default ({ strapi }: { strapi: any }) => ({
  async execute(ctx: any) {
    ...
    // Validação do body: espera version e contentHash
    ...
    // Compara a versão e o hash enviados com o termo ativo para evitar aceite desatualizado
    if (
    ...
    // 1) Verifica se já existe um registro de aceite para este usuário + versão + hash
    const existingAcceptance = await strapi.documents('api::term-list.term-list').findFirst({
      filters: {
        user: { id: userId },
    ...
    if (!existingAcceptance) {
      ...
      return { accepted: true, message: 'Novo aceite registrado e histórico antigo limpo.' };
    }

    // Se já existia o registro de aceite (mesma versão + hash), não altera o banco
