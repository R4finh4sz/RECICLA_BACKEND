...
import { MasterLoginSchema, MasterLoginInput } from '../validation/MasterLoginSchema';
...
export default ({ strapi }: { strapi: any }) => ({
  async execute(ctx: any) {
    ...
    let data: MasterLoginInput;
    try {
      data = MasterLoginSchema.parse(ctx.request.body || {});
    } catch (err) {
    ...
    try {
      ...
      strapi.log.error(`[master-login] falha ao enviar codigo 2FA por email: ${String(err?.message || err)}`);
